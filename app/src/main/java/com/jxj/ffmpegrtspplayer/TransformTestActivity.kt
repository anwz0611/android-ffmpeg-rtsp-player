...
class TransformTestActivity : AppCompatActivity(), SurfaceHolder.Callback {
    ...
    private fun createStream() {
        ...
        val streamId = FFmpegRTSPLibrary.createStream(
    ...
    private fun updateTransformStatus() {
        runOnUiThread {
            ...
            if (currentZoom > 1.0f) {
                ...
                sb.append(
                    "中心(${String.format("%.2f", zoomCenterX)},${
                        String.format(
                            "%.2f",
                            zoomCenterY
                        )
    ...
    private fun updateZoomCenterLabel() {
        zoomCenterLabelTextView.text =
            String.format("放大中心点 (X: %.2f, Y: %.2f)", zoomCenterX, zoomCenterY)
    }
